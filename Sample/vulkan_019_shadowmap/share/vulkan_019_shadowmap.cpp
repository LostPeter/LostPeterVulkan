...
void Vulkan_019_ShadowMap::ModelObjectRendIndirect::SetupVertexIndexBuffer(const ModelObjectRendPtrVector& _aRends)
{
        ...
        this->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_019_ShadowMap::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex index failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_019_ShadowMap::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
	}
}
    ...
    //1> Uniform Buffer
    {
		...
		if (this->poBuffers_ObjectCB.size() <= 0)
		{
			for (size_t j = 0; j < count_sci; j++) 
			{
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_019_ShadowMap::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer uniform: [" + nameBuffer + "] failed !";
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_019_ShadowMap::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
...
void Vulkan_019_ShadowMap::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_019_ShadowMap::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pRend->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
...
void Vulkan_019_ShadowMap::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            if (pDescriptorSetLayout == nullptr)
            {
                String msg = "*********************** Vulkan_019_ShadowMap::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            if (!pStatePipelineCompute->Init(pDescriptorSetLayout,
											 ...
											 0))
            {
                String msg = "*********************** Vulkan_019_ShadowMap::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
VKTexture* Vulkan_019_ShadowMap::findTexture(const String& nameTexture)
{
...
DescriptorSetLayout* Vulkan_019_ShadowMap::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
void Vulkan_019_ShadowMap::destroyShaders()
{   
...
void Vulkan_019_ShadowMap::createShaders()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        F_LogInfo("Vulkan_019_ShadowMap::createShaders: create shader, name: [%s], type: [%s], path: [%s] success !", 
                  shaderName.c_str(), shaderType.c_str(), shaderPath.c_str());
    }
}
VKShader* Vulkan_019_ShadowMap::findShader(const String& nameShader)
{
...
void Vulkan_019_ShadowMap::createDescriptorSets_Compute(VKStatePipelineCompute* pStatePipelineCompute, 
                                                        ModelObjectRend* pRend)
{
    VkWriteDescriptorSetVector descriptorWrites;
    ...
    void Vulkan_019_ShadowMap::drawMeshShadowMap(VkCommandBuffer& commandBuffer)
    {
        ...
        UpdateBuffer_ObjectWorld_Begin();
        {
            for (size_t i = 0; i < count_rend; i++)
            {
                ...
                instanceStart += pRend->pModelObject->countInstance;
            }
        }
