...
{
    F_Assert(_aRends.size() > 0 && "Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupVertexIndexBuffer")
    this->aRends.clear();
    this->aRends = _aRends;
    ...
    {
        F_Assert(false && "Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupVertexIndexBuffer: No vertex data !")
    }
        ...
        this->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex index failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
	}
}
...
void Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer()
{
    ...
    //1> Uniform Buffer
    {
		...
		if (this->poBuffers_ObjectCB.size() <= 0)
		{
			for (size_t j = 0; j < count_sci; j++) 
			{
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer uniform: [" + nameBuffer + "] failed !";
    ...
    //2> IndirectCommand Buffer
    {
		...
		{
			String msg = "*********************** Vulkan_013_IndirectDraw::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer indirect command: [" + nameBuffer + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
		}
    }
...
Vulkan_013_IndirectDraw::Vulkan_013_IndirectDraw(int width, int height, String name)
    ...
    for (int i = 0; i < g_Object_Count; i++)
    {
        ...
        //1> Object
        {
            ...
            //Mesh
            {
                ...
                F_Assert(pMesh != nullptr && "Vulkan_013_IndirectDraw::loadModel_Custom")
        ...
        //2> ObjectRend
        {
            ...
            for (size_t j = 0; j < count_mesh_sub_used; j++)
            {
                ...
                F_Assert(indexMeshSub >= 0 && indexMeshSub < count_mesh_sub && "Vulkan_013_IndirectDraw::loadModel_Custom")
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_013_IndirectDraw::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
...
void Vulkan_013_IndirectDraw::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_013_IndirectDraw::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pRend->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
...
void Vulkan_013_IndirectDraw::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            if (pDescriptorSetLayout == nullptr)
            {
                String msg = "*********************** Vulkan_013_IndirectDraw::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            if (!pStatePipelineCompute->Init(pDescriptorSetLayout,
											 ...
											 0))
            {
                String msg = "*********************** Vulkan_013_IndirectDraw::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
void Vulkan_013_IndirectDraw::destroyMeshes()
{
...
void Vulkan_013_IndirectDraw::createMeshes()
{
    for (int i = 0; i < g_MeshCount; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_013_IndirectDraw::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("Vulkan_013_IndirectDraw::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], mesh sub count: [%d], path: [%s] success !", 
...
void Vulkan_013_IndirectDraw::destroyTextures()
{
...
void Vulkan_013_IndirectDraw::createTextures()
{
    for (int i = 0; i < g_TextureCount; i++)
    {
        ...
        F_LogInfo("Vulkan_013_IndirectDraw::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
VKTexture* Vulkan_013_IndirectDraw::findTexture(const String& nameTexture)
{
...
DescriptorSetLayout* Vulkan_013_IndirectDraw::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
void Vulkan_013_IndirectDraw::destroyShaders()
{   
...
void Vulkan_013_IndirectDraw::createShaders()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        F_LogInfo("Vulkan_013_IndirectDraw::createShaders: create shader, name: [%s], type: [%s], path: [%s] success !", 
                  shaderName.c_str(), shaderType.c_str(), shaderPath.c_str());
    }
}
VKShader* Vulkan_013_IndirectDraw::findShader(const String& nameShader)
{
...
void Vulkan_013_IndirectDraw::createDescriptorSets_Compute(VKStatePipelineCompute* pStatePipelineCompute, 
                                                           ModelObjectRend* pRend)
{
    VkWriteDescriptorSetVector descriptorWrites;
