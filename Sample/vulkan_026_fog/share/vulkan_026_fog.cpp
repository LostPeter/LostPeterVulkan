...
void Vulkan_026_Fog::ModelObjectRendIndirect::Destroy()
{
    //Vertex/Index
...
void Vulkan_026_Fog::ModelObjectRendIndirect::CleanupSwapChain()
{
    size_t count = 0;
...
void Vulkan_026_Fog::ModelObjectRendIndirect::SetupVertexIndexBuffer(const ModelObjectRendPtrVector& _aRends)
{
    F_Assert(_aRends.size() > 0 && "Vulkan_026_Fog::ModelObjectRendIndirect::SetupVertexIndexBuffer")
    this->aRends.clear();
    this->aRends = _aRends;
    ...
    {
        F_Assert(false && "Vulkan_026_Fog::ModelObjectRendIndirect::SetupVertexIndexBuffer: No vertex data !")
    }
        ...
        this->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_026_Fog::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex index failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_026_Fog::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
	}
}

void Vulkan_026_Fog::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer()
{
    ...
    //1> Uniform Buffer
    {
		...
		if (this->poBuffers_ObjectCB.size() <= 0)
		{
			for (size_t j = 0; j < count_sci; j++) 
			{
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_026_Fog::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer uniform: [" + nameBuffer + "] failed !";
    ...
    //2> IndirectCommand Buffer
    {
		...
		{
			String msg = "*********************** Vulkan_026_Fog::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer indirect command: [" + nameBuffer + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
		}
    }
...
void Vulkan_026_Fog::ModelObjectRendIndirect::UpdateUniformBuffer()
{
    int count_index = 0;
    size_t count_rend = this->aRends.size();
    for (size_t i = 0; i < count_rend; i++)
...
void Vulkan_026_Fog::ModelObjectRendIndirect::UpdateIndirectCommandBuffer()
{
    this->indirectCommandCBs.clear();
...
Vulkan_026_Fog::Vulkan_026_Fog(int width, int height, String name)
...
void Vulkan_026_Fog::setUpEnabledFeatures()
{
    ...
    else
    {
        ...
        F_LogError("*********************** Vulkan_026_Fog::setUpEnabledFeatures: multiDrawIndirect is not supported !");
    }
}

void Vulkan_026_Fog::createDescriptorSetLayout_Custom()
{
    VulkanWindow::createDescriptorSetLayout_Custom();
}

void Vulkan_026_Fog::createCamera()
{
    this->pCamera = new FCamera();
    cameraReset();
}
void Vulkan_026_Fog::cameraReset()
{
...
void Vulkan_026_Fog::loadModel_Custom()
{
    createMeshes();
    createTextures();
    ...
    for (int i = 0; i < g_Object_Count; i++)
    {
        ...
        //1> Object
        {
            ...
            //Mesh
            {
                ...
                F_Assert(pMesh != nullptr && "Vulkan_026_Fog::loadModel_Custom")
        ...
        //2> ObjectRend
        {
            ...
            for (size_t j = 0; j < count_mesh_sub_used; j++)
            {
                ...
                F_Assert(indexMeshSub >= 0 && indexMeshSub < count_mesh_sub && "Vulkan_026_Fog::loadModel_Custom")
...
void Vulkan_026_Fog::createIndirectCommands()
{

}

void Vulkan_026_Fog::createCustomCB()
{
    rebuildInstanceCBs(true);
}
void Vulkan_026_Fog::rebuildInstanceCBs(bool isCreateVkBuffer)
{   
    VkDeviceSize bufferSize;
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_026_Fog::createCustomCB: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
...
void Vulkan_026_Fog::createCustomBeforePipeline()
{
...
void Vulkan_026_Fog::createGraphicsPipeline_Custom()
{
    //1> Viewport
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Can not find shader used !";
            F_LogError(msg.c_str());
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Can not find DescriptorSetLayoutNames by name: " + pRend->pPipelineGraphics->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pRend->pPipelineGraphics->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Can not find PipelineLayout by name: " + pRend->pPipelineGraphics->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Failed to create pipeline graphics wire frame: " + pRend->nameObjectRend;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            F_LogInfo("Vulkan_026_Fog::createGraphicsPipeline_Custom: Object: [%s] Create pipeline graphics wire frame success !", pRend->nameObjectRend.c_str());
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Failed to create pipeline graphics: " + pRend->nameObjectRend;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            F_LogInfo("Vulkan_026_Fog::createGraphicsPipeline_Custom: Object: [%s] Create pipeline graphics graphics success !", pRend->nameObjectRend.c_str());
        }
    }
}
void Vulkan_026_Fog::createComputePipeline_Custom()
{
    size_t count_rend = this->m_aModelObjectRends_All.size();
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Can not find shader used !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        ...
        {
            String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Pipeline count is not equal shader count !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Can not find DescriptorSetLayoutNames by name: " + p->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + p->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Can not find PipelineLayout by name: " + p->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + p->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
void Vulkan_026_Fog::destroyMeshes()
{
...
void Vulkan_026_Fog::createMeshes()
{
    for (int i = 0; i < g_MeshCount; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_026_Fog::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("Vulkan_026_Fog::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], mesh sub count: [%d], path: [%s] success !", 
                  nameMesh.c_str(), nameVertexType.c_str(), nameMeshType.c_str(), nameGeometryType.c_str(), (int)pMesh->aMeshSubs.size(), pathMesh.c_str());
    }
}
Mesh* Vulkan_026_Fog::findMesh(const String& nameMesh)
{
...
void Vulkan_026_Fog::destroyTextures()
{
...
void Vulkan_026_Fog::createTextures()
{
    for (int i = 0; i < g_TextureCount; i++)
    {
        ...
        F_LogInfo("Vulkan_026_Fog::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
VKTexture* Vulkan_026_Fog::findTexture(const String& nameTexture)
{
...
void Vulkan_026_Fog::destroyDescriptorSetLayouts()
{
...
void Vulkan_026_Fog::createDescriptorSetLayouts()
{
    for (int i = 0; i < g_DescriptorSetLayoutCount; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_026_Fog::createDescriptorSetLayouts: Failed to create descriptor set layout: " + nameLayout;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("Vulkan_026_Fog::createDescriptorSetLayouts: create DescriptorSetLayout: [%s] success !", nameLayout.c_str());
    }
}
VkDescriptorSetLayout Vulkan_026_Fog::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
StringVector* Vulkan_026_Fog::findDescriptorSetLayoutNames(const String& nameDescriptorSetLayout)
{
...
void Vulkan_026_Fog::destroyShaderModules()
{   
...
void Vulkan_026_Fog::createShaderModules()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        F_LogInfo("Vulkan_026_Fog::createShaderModules: create shader, name: [%s], type: [%s], path: [%s] success !", 
                  shaderName.c_str(), shaderType.c_str(), shaderPath.c_str());
    }
}
VkShaderModule Vulkan_026_Fog::findShaderModule(const String& nameShaderModule)
{
...
void Vulkan_026_Fog::destroyPipelineLayouts()
{
...
void Vulkan_026_Fog::createPipelineLayouts()
{
    for (int i = 0; i < g_DescriptorSetLayoutCount; i++)
    {
        ...
        if (vkDescriptorSetLayout == VK_NULL_HANDLE)
        {
            F_LogError("*********************** Vulkan_026_Fog::createPipelineLayouts: Can not find DescriptorSetLayout by name: [%s]", nameDescriptorSetLayout.c_str());
            return;
        }
        ...
        {
            F_LogError("*********************** Vulkan_026_Fog::createPipelineLayouts: createVkPipelineLayout failed !");
            return;
        }
...
VkPipelineLayout Vulkan_026_Fog::findPipelineLayout(const String& namePipelineLayout)
{
...
void Vulkan_026_Fog::createDescriptorSets_Custom()
{
    //1> Object Rend
...
void Vulkan_026_Fog::createDescriptorSets_Graphics(VkDescriptorSetVector& poDescriptorSets, 
                                                   ...
                                                   ModelObjectRendIndirect* pRendIndirect)
{
    ...
    F_Assert(pDescriptorSetLayoutNames != nullptr && "Vulkan_026_Fog::createDescriptorSets_Graphics")
    ...
    {   
        ...
        for (size_t p = 0; p < count_names; p++)
        {
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createDescriptorSets_Graphics: Graphics: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
void Vulkan_026_Fog::createDescriptorSets_Compute(VKPipelineCompute* pPipelineCompute, 
                                                  ModelObjectRend* pRend)
{
    StringVector* pDescriptorSetLayoutNames = pPipelineCompute->poDescriptorSetLayoutNames;
    F_Assert(pDescriptorSetLayoutNames != nullptr && "Vulkan_026_Fog::createDescriptorSets_Compute")
    ...
    for (size_t p = 0; p < count_names; p++)
    {
        ...
        {
            String msg = "*********************** Vulkan_026_Fog::createDescriptorSets_Compute: Compute: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
    }  
...
void Vulkan_026_Fog::updateCompute_BeforeRender_Custom(VkCommandBuffer& commandBuffer)
{
    size_t count_object_rend = this->m_aModelObjectRends_All.size();
    for (size_t i = 0; i < count_object_rend; i++)
    {
...
void Vulkan_026_Fog::updateCBs_Custom()
{
    //1> Object Rend
...
void Vulkan_026_Fog::updateRenderPass_SyncComputeGraphics(VkCommandBuffer& commandBuffer)
{
    size_t count_object_rend = this->m_aModelObjectRends_All.size();
    for (size_t i = 0; i < count_object_rend; i++)
    {
...
bool Vulkan_026_Fog::beginRenderImgui()
{
    ImGui_ImplVulkan_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("Vulkan_026_Fog", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void Vulkan_026_Fog::modelConfig()
{
...
void Vulkan_026_Fog::endRenderImgui()
{
    VulkanWindow::endRenderImgui();

}

void Vulkan_026_Fog::drawMeshDefault_Custom(VkCommandBuffer& commandBuffer)
{   
...
void Vulkan_026_Fog::drawModelObjectRendIndirects(VkCommandBuffer& commandBuffer, ModelObjectRendPtrVector& aRends)
{
...
void Vulkan_026_Fog::drawModelObjectRendIndirect(VkCommandBuffer& commandBuffer, ModelObjectRendIndirect* pRendIndirect)
{
...
void Vulkan_026_Fog::drawModelObjectRends(VkCommandBuffer& commandBuffer, ModelObjectRendPtrVector& aRends)
{
    size_t count_rend = aRends.size();
    for (size_t i = 0; i < count_rend; i++)
    {
...
void Vulkan_026_Fog::drawModelObjectRend(VkCommandBuffer& commandBuffer, ModelObjectRend* pRend)
{
    ModelObject* pModelObject = pRend->pModelObject;
...
void Vulkan_026_Fog::cleanupCustom()
{   
    destroyTextures();
    destroyMeshes();
...
void Vulkan_026_Fog::cleanupSwapChain_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void Vulkan_026_Fog::recreateSwapChain_Custom()
{   
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
