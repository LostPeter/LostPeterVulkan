    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_026_Fog::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
...
void Vulkan_026_Fog::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_026_Fog::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pRend->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
...
void Vulkan_026_Fog::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            if (pDescriptorSetLayout == nullptr)
            {
                String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            if (!pStatePipelineCompute->Init(pDescriptorSetLayout,
											 ...
											 0))
            {
                String msg = "*********************** Vulkan_026_Fog::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
DescriptorSetLayout* Vulkan_026_Fog::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
void Vulkan_026_Fog::createDescriptorSets_Compute(VKStatePipelineCompute* pStatePipelineCompute, 
                                                  ModelObjectRend* pRend)
{
    VkWriteDescriptorSetVector descriptorWrites;
