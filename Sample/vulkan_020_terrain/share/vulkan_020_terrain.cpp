...
void Vulkan_020_Terrain::ModelObjectRendIndirect::SetupVertexIndexBuffer(const ModelObjectRendPtrVector& _aRends)
{
        ...
        this->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_020_Terrain::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex index failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_020_Terrain::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
	}
}
...
void Vulkan_020_Terrain::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer()
{
    ...
    //1> Uniform Buffer
    {
		...
		if (this->poBuffers_ObjectCB.size() <= 0)
		{
			for (size_t j = 0; j < count_sci; j++) 
			{
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_020_Terrain::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer uniform: [" + nameBuffer + "] failed !";
    ...
    //2> IndirectCommand Buffer
    {
		...
		{
			String msg = "*********************** Vulkan_020_Terrain::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer indirect command: [" + nameBuffer + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
		}
	}
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_020_Terrain::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
...
void Vulkan_020_Terrain::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_020_Terrain::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pRend->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
...
void Vulkan_020_Terrain::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            if (pDescriptorSetLayout == nullptr)
            {
                String msg = "*********************** Vulkan_020_Terrain::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            if (!pStatePipelineCompute->Init(pDescriptorSetLayout,
											 ...
											 0))
            {
                String msg = "*********************** Vulkan_020_Terrain::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
VKTexture* Vulkan_020_Terrain::findTexture(const String& nameTexture)
{
...
DescriptorSetLayout* Vulkan_020_Terrain::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
void Vulkan_020_Terrain::destroyShaders()
{   
...
void Vulkan_020_Terrain::createShaders()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        F_LogInfo("Vulkan_020_Terrain::createShaders: create shader, name: [%s], type: [%s], path: [%s] success !", 
                  shaderName.c_str(), shaderType.c_str(), shaderPath.c_str());
    }
}
VKShader* Vulkan_020_Terrain::findShader(const String& nameShader)
{
...
void Vulkan_020_Terrain::createDescriptorSets_Compute(VKStatePipelineCompute* pStatePipelineCompute, 
                                                      ModelObjectRend* pRend)
{
    VkWriteDescriptorSetVector descriptorWrites;
...
void Vulkan_020_Terrain::updateCompute_BeforeRender_Custom(VkCommandBuffer& commandBuffer)
{
    size_t count_object_rend = this->m_aModelObjectRends_All.size();
    for (size_t i = 0; i < count_object_rend; i++)
    {
