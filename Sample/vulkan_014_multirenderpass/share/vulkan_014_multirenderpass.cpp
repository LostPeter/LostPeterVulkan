...
void Vulkan_014_MultiRenderPass::ModelObjectRendIndirect::SetupVertexIndexBuffer(const ModelObjectRendPtrVector& _aRends)
{
        ...
        this->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_014_MultiRenderPass::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex index failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_014_MultiRenderPass::ModelObjectRendIndirect::SetupVertexIndexBuffer: create buffer vertex failed: [%s] !", this->nameObjectRendIndirect.c_str());
			return;
		}
	}
}
...
void Vulkan_014_MultiRenderPass::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer()
{
    ...
    //1> Uniform Buffer
    {
		...
		{		
			for (size_t j = 0; j < count_sci; j++) 
			{
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_014_MultiRenderPass::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer uniform: [" + nameBuffer + "] failed !";
    ...
    //2> IndirectCommand Buffer
    {
		...
		{
			String msg = "*********************** Vulkan_014_MultiRenderPass::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer indirect command: [" + nameBuffer + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
		}
    }
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_014_MultiRenderPass::createCustomCB: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            pRend->pPipelineGraphics->poPipeline_WireFrame = createVkGraphicsPipeline("PipelineGraphics-Wire-" + pRend->nameObjectRend,
                                                                                      ...
                                                                                      pRend->pPipelineGraphics->pRenderPass->poRenderPass, pRend->pPipelineGraphics->poPipelineLayout, aViewports, aScissors, this->cfg_aDynamicStates,
...
void Vulkan_014_MultiRenderPass::destroyMeshes()
{
...
void Vulkan_014_MultiRenderPass::createMeshes()
{
    for (int i = 0; i < g_Mesh_Count; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_014_MultiRenderPass::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("Vulkan_014_MultiRenderPass::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], mesh sub count: [%d], path: [%s] success !", 
                  nameMesh.c_str(), nameVertexType.c_str(), nameMeshType.c_str(), nameGeometryType.c_str(), (int)pMesh->aMeshSubs.size(), pathMesh.c_str());
    }
}
Mesh* Vulkan_014_MultiRenderPass::findMesh(const String& nameMesh)
{
...
void Vulkan_014_MultiRenderPass::destroyTextures()
{
...
void Vulkan_014_MultiRenderPass::createTextures()
{
    for (int i = 0; i < g_Texture_Count; i++)
    {
        ...
        F_LogInfo("Vulkan_014_MultiRenderPass::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
VKTexture* Vulkan_014_MultiRenderPass::findTexture(const String& nameTexture)
{
...
void Vulkan_014_MultiRenderPass::destroyShaders()
{   
...
void Vulkan_014_MultiRenderPass::createShaders()
{
    for (int i = 0; i < g_Shader_Count; i++)
    {
        ...
        F_LogInfo("Vulkan_014_MultiRenderPass::createShaders: create shader, name: [%s], type: [%s], path: [%s] success !", 
                  shaderName.c_str(), shaderType.c_str(), shaderPath.c_str());
    }
}
VKShader* Vulkan_014_MultiRenderPass::findShader(const String& nameShader)
{
