...
bool Vulkan_009_Instancing::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load 
    FMeshData meshData;
        ...
        pModelObject->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_009_Instancing::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_009_Instancing::loadModel_VertexIndex: create buffer vertex failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
...
void Vulkan_009_Instancing::createCustomBeforePipeline()
{
    //1> Shader
    createShaders();
}  
void Vulkan_009_Instancing::createGraphicsPipeline_Custom()
{
    //1> Viewport
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_009_Instancing::createGraphicsPipeline_Custom: Failed to create pipeline stencil, name: " + nameStatePipelineGraphics_Stencil;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        ...
        {
            String msg = "*********************** Vulkan_009_Instancing::createGraphicsPipeline_Custom: Failed to create pipeline outline, name: " + nameStatePipelineGraphics_Outline;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
    }
...
void Vulkan_009_Instancing::destroyShaders()
{   
...
void Vulkan_009_Instancing::createShaders()
{
    ...
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        F_LogInfo("Vulkan_009_Instancing::createShaders: create shader [%s] success !", pathVert.c_str());
        ...
        F_LogInfo("Vulkan_009_Instancing::createShaders: create shader [%s] success !", pathFrag.c_str());
    }
}
VKShader* Vulkan_009_Instancing::findShader(const String& nameShader)
{
