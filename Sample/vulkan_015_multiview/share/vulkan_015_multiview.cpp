...
void Vulkan_015_MultiView::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer()
{
    ...
    //1> Uniform Buffer
    {
		...
		if (this->poBuffers_ObjectCB.size() <= 0)
		{
			for (size_t j = 0; j < count_sci; j++) 
			{
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_015_MultiView::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer uniform: [" + nameBuffer + "] failed !";
    ...
    //2> IndirectCommand Buffer
    {
		...
		{
			String msg = "*********************** Vulkan_015_MultiView::ModelObjectRendIndirect::SetupUniformIndirectCommandBuffer: create buffer indirect command: [" + nameBuffer + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
		}
    }
    ...
    for (int i = 0; i < g_Object_Count; i++)
    {
        ...
        //2> ObjectRend
        {
            ...
            for (size_t j = 0; j < count_mesh_sub_used; j++)
            {
                ...
                pRend->nameRenderPass = g_ObjectRend_NameRenderPasses[nIndexObjectRend];
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_015_MultiView::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
					F_LogError(msg.c_str());
					throw std::runtime_error(msg);
				}
...
void Vulkan_015_MultiView::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_015_MultiView::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pRend->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }

            pRend->poRenderPass = findMultiRenderPass(pRend->nameRenderPass);
            if (pRend->poRenderPass == nullptr)
            {
                String msg = "*********************** Vulkan_015_MultiView::createGraphicsPipeline_Custom: Can not find VKMultiRenderPass by name: " + pRend->nameRenderPass;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            addRenderPass2ModelObjectRendMap(pRend->poRenderPass, pRend);
			...
			FMeshVertexType typeVertex = F_MeshVertex_Count;
            if (pRend->pMeshSub != nullptr)
            {
				typeVertex = pRend->pMeshSub->poTypeVertex;
            }
            ...
            pRend->poStatePipelineGraphics = createStatePipelineGraphics(nameStatePipelineGraphics,
																		 ...
																		 pRend->poRenderPass->poRenderPass, aViewports, aScissors, this->poDynamicStates,
...
void Vulkan_015_MultiView::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count_rend; i++)
    {
        ...
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            if (pDescriptorSetLayout == nullptr)
            {
                String msg = "*********************** Vulkan_015_MultiView::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            if (!pStatePipelineCompute->Init(pDescriptorSetLayout,
											 ...
											 0))
            {
                String msg = "*********************** Vulkan_015_MultiView::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
void Vulkan_015_MultiView::destroyMeshes()
{
...
void Vulkan_015_MultiView::createMeshes()
{
    for (int i = 0; i < g_Mesh_Count; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_015_MultiView::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("Vulkan_015_MultiView::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], mesh sub count: [%d], path: [%s] success !", 
...
void Vulkan_015_MultiView::destroyTextures()
{
...
void Vulkan_015_MultiView::createTextures()
{
    for (int i = 0; i < g_Texture_Count; i++)
    {
        ...
        F_LogInfo("Vulkan_015_MultiView::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
VKTexture* Vulkan_015_MultiView::findTexture(const String& nameTexture)
{
...
DescriptorSetLayout* Vulkan_015_MultiView::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
{    
    for (int i = 0; i < g_RenderPass_Count; i++)
    {
        ...
        VKMultiRenderPass* pMultiRenderPass = new VKMultiRenderPass(nameRenderPass, isDefault, true);
...
void Vulkan_015_MultiView::createDescriptorSets_Custom()
{
    ...
    for (size_t i = 0; i < count_object_rend; i++)
    {
        ...
        //Pipeline Graphics
        {
			...
			if (pRend->isMultiView)
			{
				createDescriptorSets_Graphics(pRend->poStatePipelineGraphics2->poDescriptorSets, pRend, nullptr);
			}
        }   
...
void Vulkan_015_MultiView::createDescriptorSets_Compute(VKStatePipelineCompute* pStatePipelineCompute, 
                                                        ModelObjectRend* pRend)
{
    VkWriteDescriptorSetVector descriptorWrites;
...
void Vulkan_015_MultiView::addRenderPass2ModelObjectRendMap(VKMultiRenderPass* pRenderPass, ModelObjectRend* pRend)
{   
    if (pRend->isTransparent)
    {
...
void Vulkan_015_MultiView::addRenderPass2ModelObjectRendMap(MultiRenderPass2ObjectRendsMap& mapRP2OR, VKMultiRenderPass* pRenderPass, ModelObjectRend* pRend)
{
...
void Vulkan_015_MultiView::drawModelObjectRendByRenderPass(VkCommandBuffer& commandBuffer, VKMultiRenderPass* pRenderPass)
{
...
void Vulkan_015_MultiView::drawModelObjectRendView(VkCommandBuffer& commandBuffer, ModelObjectRend* pRend, VKStatePipelineGraphics* pStatePipelineGraphics)
{
    ModelObject* pModelObject = pRend->pModelObject;
