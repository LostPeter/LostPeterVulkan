...
/////////////////////////// ModelMesh ///////////////////////////
bool Vulkan_011_Texturing::ModelMesh::LoadMesh(bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load
    FMeshData meshData;
        ...
        this->poIndexBuffer_Data != nullptr)
    {
		...
		{
			F_LogError("*********************** Vulkan_011_Texturing::ModelMesh::LoadMesh: create buffer vertex index failed: [%s] !", this->nameMesh.c_str());
			return false;
		}
    }
	else
	{
		...
		{
			F_LogError("*********************** Vulkan_011_Texturing::ModelMesh::LoadMesh: create buffer vertex failed: [%s] !", this->nameMesh.c_str());
			return false;
		}
	}
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (isCreateVkBuffer)
        {
            ...
            for (size_t j = 0; j < count_sci; j++) 
            {
				...
				if (!pBufferUniform)
				{
					String msg = "*********************** Vulkan_011_Texturing::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
...
void Vulkan_011_Texturing::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        //[2] Pipeline Graphics
        {
            ...
            {
                String msg = "*********************** Vulkan_011_Texturing::createGraphicsPipeline_Custom: Can not find DescriptorSetLayout by name: " + pModelObject->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            {
                String msg = "*********************** Vulkan_011_Texturing::createGraphicsPipeline_Custom: Failed to create pipeline graphics, name: " + nameStatePipelineGraphics;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
...
void Vulkan_011_Texturing::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        for (size_t j = 0; j < count_pipeline; j ++)
        {
            ...
            if (pDescriptorSetLayout == nullptr)
            {
                String msg = "*********************** Vulkan_011_Texturing::createComputePipeline_Custom: Can not find DescriptorSetLayout by name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
            ...
            if (!pStatePipelineCompute->Init(pDescriptorSetLayout,
											 ...
											 0))
            {
                String msg = "*********************** Vulkan_011_Texturing::createComputePipeline_Custom: Create compute pipeline failed, PipelineLayout name: " + pStatePipelineCompute->nameDescriptorSetLayout;
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        }
...
void Vulkan_011_Texturing::destroyMeshes()
{
...
void Vulkan_011_Texturing::createMeshes()
{
    for (int i = 0; i < g_MeshCount; i++)
    {
        ...
        {
            String msg = "*********************** Vulkan_011_Texturing::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("Vulkan_011_Texturing::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], path: [%s] success !", 
                  nameMesh.c_str(), nameVertexType.c_str(), nameMeshType.c_str(), nameGeometryType.c_str(), pathMesh.c_str());
    }
}
Vulkan_011_Texturing::ModelMesh* Vulkan_011_Texturing::findMesh(const String& nameMesh)
{
...
void Vulkan_011_Texturing::destroyTextures()
{
...
void Vulkan_011_Texturing::createTextures()
{
    for (int i = 0; i < g_TextureCount; i++)
    {
        ...
        F_LogInfo("Vulkan_011_Texturing::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
VKTexture* Vulkan_011_Texturing::findTexture(const String& nameTexture)
{
...
DescriptorSetLayout* Vulkan_011_Texturing::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
