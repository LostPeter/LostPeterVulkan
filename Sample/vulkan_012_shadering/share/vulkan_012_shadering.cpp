...
void Vulkan_012_Shadering::loadModel_Custom()
{
    ...
    for (int i = 0; i < g_ModelCount; i++)
    {
        ...
        {   
            std::string msg = "Vulkan_012_Shadering::loadModel_Custom: Failed to load textures !";
            Util_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
bool Vulkan_012_Shadering::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTranformLocal, const glm::mat4& matTransformLocal)
{
    ...
    Util_LogInfo("Vulkan_012_Shadering::loadModel_VertexIndex: load model [%s] success, Vertex count: [%d], Index count: [%d] !", 
...
bool Vulkan_012_Shadering::loadModel_Texture(ModelObject* pModelObject)
{
...
void Vulkan_012_Shadering::createPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            std::string msg = "Vulkan_012_Shadering::createPipeline_Custom: Can not find PipelineLayout by name: " + pModelObject->nameDescriptorSetLayout;
            Util_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        ...
        {
            std::string msg = "Vulkan_012_Shadering::createPipeline_Custom: Failed to create pipeline wire frame !";
            Util_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        ...
        {
            std::string msg = "Vulkan_012_Shadering::createPipeline_Custom: Failed to create pipeline !";
            Util_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
    }
...
void Vulkan_012_Shadering::createModelTextures()
{
    for (int i = 0; i < g_TextureCount; i++)
    {
        ...
        Util_LogInfo("Vulkan_012_Shadering::createModelTextures: create texture: [%s], type: [%s], path: [%s] success !", nameTexture.c_str(), nameType.c_str(), pathTextures.c_str());
    }
}
