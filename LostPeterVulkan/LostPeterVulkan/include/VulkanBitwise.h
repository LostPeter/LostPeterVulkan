...
#ifndef _VULKAN_BITWISE_H_
#define _VULKAN_BITWISE_H_
...
namespace LostPeter
{
    ...
    {
    public:
        static UTIL_FORCEINLINE uint16 Swap16(uint16 arg)
        {
#if UTIL_COMPILER == VULKAN_COMPILER_MSVC && VULKAN_COMPILER_VERSION >= 1310
            return _byteswap_ushort(arg);
#elif (UTIL_COMPILER == VULKAN_COMPILER_CLANG && __has_builtin(__builtin_bswap16)) || (UTIL_COMPILER == VULKAN_COMPILER_GNUC && VULKAN_COMPILER_VERSION >= 480)
        ...
        static UTIL_FORCEINLINE uint32 Swap32(uint32 arg)
        {
#if UTIL_COMPILER == VULKAN_COMPILER_MSVC && VULKAN_COMPILER_VERSION >= 1310
            return _byteswap_ulong(arg);
#elif (UTIL_COMPILER == VULKAN_COMPILER_CLANG && __has_builtin(__builtin_bswap32)) || (UTIL_COMPILER == VULKAN_COMPILER_GNUC && VULKAN_COMPILER_VERSION >= 430)
        ...
        static UTIL_FORCEINLINE uint64 Swap64(uint64 arg)
        {
#if UTIL_COMPILER == VULKAN_COMPILER_MSVC && VULKAN_COMPILER_VERSION >= 1310
            return _byteswap_uint64(arg);
#elif (UTIL_COMPILER == VULKAN_COMPILER_CLANG && __has_builtin(__builtin_bswap64)) || (UTIL_COMPILER == VULKAN_COMPILER_GNUC && VULKAN_COMPILER_VERSION >= 430)
        ...
        static UTIL_FORCEINLINE void SwapBuffer(void * pData, size_t size)
        {
            char swapByte;
        ...
        static UTIL_FORCEINLINE void SwapChunks(void* pData, size_t size, size_t count)
        {
		...
		template<typename T>																											
		static UTIL_FORCEINLINE bool IsPow2(T n)		
		{
			return (n & (n-1)) == 0;
		}
							
		template<typename T>													
		static UTIL_FORCEINLINE uint32 GetBitShift(T mask)								
		{
			if (mask == 0)
				return 0;
		...
		static UTIL_FORCEINLINE DestT	ConvertBitPattern(SrcT srcValue, SrcT srcBitMask, DestT destBitMask)
		{
