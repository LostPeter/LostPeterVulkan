...
{
    VulkanStreamIndex::VulkanStreamIndex(VulkanDevice* pDevice, 
                                         uint32 nTarget, 
        ...
        , m_vkBuffer(nullptr)
    {
        ...
        if (m_eStreamUsage == E_StreamUsage_Dynamic)
        {
            ...
            m_pStreamShadow = new VulkanStreamIndex(pDevice, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, eStreamIndex, nIndexCount, E_StreamUsage_Static, true, false);
        }
    }
    ...
    void VulkanStreamIndex::ReadData(uint32 nOffset, uint32 nLength, void* pDest)
    {
        StreamLock streamLock(this, nOffset, nLength, E_StreamLock_ReadOnly);
        memcpy(pDest, streamLock.m_pData, nLength);
    }

	void VulkanStreamIndex::WriteData(uint32 nOffset, uint32 nLength, const void* pSource, bool bIsDiscardWholeStream /*= false*/)
    {
	...
	void VulkanStreamIndex::CopyData(Stream& streamSrc, uint32 nSrcOffset, uint32 nDstOffset, uint32 nLength, bool bIsDiscardWholeStream /*= false*/)
    {
	...
	void* VulkanStreamIndex::lockImpl(uint32 nOffset, uint32 nLength, EStreamLockType eStreamLock)
    {
        if (m_pStreamShadow)
