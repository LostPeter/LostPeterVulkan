...
namespace LostPeterPluginRendererVulkan
{
    ...
    void VulkanDevice::getDeviceLayersAndExtensions(bool bIsEnableValidationLayers,
                                                    ConstCharPtrVector& outDeviceLayers, 
                                                    ConstCharPtrVector& outDeviceExtensions)
    {
        uint32 count = 0;
        vkEnumerateDeviceLayerProperties(this->m_vkPhysicalDevice, &count, nullptr);
        std::vector<VkLayerProperties> properties(count);
        vkEnumerateDeviceLayerProperties(this->m_vkPhysicalDevice, &count, properties.data());
        ...
        for (size_t i = 0; i < deviceLayerExtensions.size(); ++i)
        {
            if (i == 0) 
            {
                EnumerateDeviceExtensionProperties(this->m_vkPhysicalDevice, nullptr, deviceLayerExtensions[i]);
            }
            else 
            {
