...
namespace LostPeterPluginRendererVulkan
{
                                       ...
                                       const String& strWndTitle /*= "Render Window"*/)
    {
        ...
        //3> Init StreamManager
        if (!initStreamManager())
        {
            F_LogError("*********************** VulkanRenderer::Init: 3> Failed to init stream manager !");
            return nullptr;
        }
        F_LogInfo("VulkanRenderer::Init: 3> Init stream manager success !");

        //4> Init ShaderProgramManager
        if (!initShaderProgramManager())
        {
            F_LogError("*********************** VulkanRenderer::Init: 4> Failed to init shader program manager !");
            return nullptr;
        }
        F_LogInfo("VulkanRenderer::Init: 4> Init shader program manager success !");

        //5> Init TextureManager
        if (!initTextureManager())
        {
            F_LogError("*********************** VulkanRenderer::Init: 5> Failed to init texture manager !");
            return nullptr;
        }
        F_LogInfo("VulkanRenderer::Init: 5> Init texture manager success !");
        ...
        bool VulkanRenderer::initStreamManager()
        {
            VulkanDevice* pDevice = GetDevice();
            m_pStreamManager = new VulkanStreamManager(pDevice);
            
            F_LogInfo("VulkanRenderer::initStreamManager: Success to init stream manager !");
            return true;
        }
