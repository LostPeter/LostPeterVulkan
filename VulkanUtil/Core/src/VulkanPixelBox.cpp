...
namespace LostPeter
{
	...
	VulkanPixelBox::VulkanPixelBox(const VulkanMathBox& extents, VulkanPixelFormatType ePixelFormat, void* pPixelData /*= nullptr*/)
		: VulkanMathBox(extents)
	...
	VulkanPixelBox::VulkanPixelBox(size_t width, size_t height, size_t depth, VulkanPixelFormatType ePixelFormat, void* pPixelData /*= nullptr*/)
		: VulkanMathBox(0, 0, 0, width, height, depth)
		, m_pData((uint8*)pPixelData)
		, m_ePixelFormat(ePixelFormat)
	{
		SetConsecutive();
	...
	VulkanPixelBox VulkanPixelBox::GetSubVolume(const VulkanMathBox& def, bool resetOrigin /*= true*/) const
	{
		if (VulkanPixelFormat::IsCompressed(m_ePixelFormat))
		{
			...
			Util_LogError("VulkanPixelBox::GetSubVolume: Cannot return subvolume of compressed PixelBuffer");
			assert(false && "VulkanPixelBox::GetSubVolume");
		}

		if (!Contains(def))
		{
			Util_LogError("VulkanPixelBox::GetSubVolume: Bounds out of range");
			assert(false && "VulkanPixelBox::GetSubVolume");
		}
	...
	Color VulkanPixelBox::GetColorAt(size_t x, size_t y, size_t z) const
	{
		Color cv;
	...
	void VulkanPixelBox::SetColorAt(size_t x, size_t y, size_t z, const Color &cv)
	{
		size_t pixelSize = VulkanPixelFormat::GetPixelFormatElemBytes(m_ePixelFormat);
        size_t pixelOffset = pixelSize * (z * m_nSlicePitch + y * m_nRowPitch + x);
